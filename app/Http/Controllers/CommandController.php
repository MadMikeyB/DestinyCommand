...
class CommandController extends Controller
{
    ...
    public function parseRequest(Request $request)
    {
        try {
            ...
            if ($request->header('Nightbot-Channel')) {
                ...
                $oCommand->setPlatform($request->input('platform', 'twitch'));

                // Bot
                $oCommand->setBot($request->input('bot', 'nightbot'));

                // Channel
                if ($request->has('channel')) {
                    $oCommand->setChannel($request->input('channel'));
                }
            }
            ...
            if ($request->header('Nightbot-User')) {
                ...
                $oCommand->setUser($request->input('user', 'System'));
            }

            // Token
            if ($request->has('token')) {
                $oCommand->setToken($request->input('token'));
            }

            // Dont display username
            $bUsername = true;
            if ($request->has('nousername')) {
                $bUsername = false;
            }

            // Dont display gamertag
            $bGamertag = true;
            if ($request->has('nogamertag')) {
                $bGamertag = false;
            }

            $oCommand->setDefaultConsole($request->input('default_console', 'xbox'));
            $oCommand->setResponseUser($request->input('response_user', ''));

            // Query (Default to default_info)
            $oCommand->setQuery($request->input('query'));
            ...
            // Format Discord
            if ($this->command->platform == 'discord' && isset($this->command->responseUser)) {
                $this->command->responseUser = $this->formatDiscord($this->command->responseUser);
            }
                        ...
                        if (isset($aRes['response'][$strKey])) {
                            ...
                            foreach ($aRes['response'][$strKey] as $iCharacterId => $aCharacter) {
                                ...
                                foreach ($aCharacter as $x) {
                                    switch (true) {
                                        case $x instanceof EquipmentItem:
                                            ...
                                            if (isset($oCharacterItem->light) && $oCharacterItem->light > 50) {
                                                $strCharacterRes .= ' ['.$oCharacterItem->light.']';
                                            }
                                ...
                                if ($strCharacterRes !== false) {
                                    ...
                                    if ($iCharacterId != 0 && isset($this->prep[$iCharacterId])) {
                                        $strRes .= $aClasses[$this->prep[$iCharacterId]].': ';
                                    }
    ...
    private function runCommand()
    {
        ...
        foreach ($this->command->query->actions as $oAction) {
            if ($oAction->provider == 'plain_text') {
                continue;
            }
            $strClass = 'App\Providers\\'.$oAction->provider;
            if (! isset(${$oAction->provider})) {
                ${$oAction->provider} = (new $strClass);
            }
            $this->prep = ${$oAction->provider}->fetch($oAction, ['players' => $aPlayers], true);
        }
        ...
        foreach ($this->command->query->actions as $oAction) {
            if ($oAction->key == 'setplayer') {
                ...
                if ($this->isModerator()) {
                    ...
                    $oAction->text = 'Not allowed to set Xur location';
                }
            }

            $x = $oAction->provider == 'plain_text' ? ['text' => [$oAction->text]] : ${$oAction->provider}->fetch($oAction, ['players' => $aPlayers], false);
            if (is_array($x)) {
    ...
    {
        $aModeratorKeys = config('destinycommand.moderator_keys', []);
    ...
    public function getLinkedProfiles(BungieNetAccount $oBungieNetAccount, $bLastPlayedPlayer = false)
    {
        ...
        foreach ($aLinkedProfilesResults as $aLinkedProfiles) {
            break;
        }
        ...
        if (! isset($aLinkedProfiles->profiles) || empty($aLinkedProfiles->profiles)) {
            ...
            if ($bLastPlayedPlayer) {
                ...
                return $aLinkedProfiles->profiles;
            }
        }
    }
    ...
    public function getPlayers()
    {
        ...
        if (! empty($this->command->query->gamertags)) {
            ...
            foreach ($this->command->query->gamertags as $i => $strGamertag) {
                ...
                [$strDisplayName, $iDisplayNameCode] = explode('#', $strGamertag);
        ...
        } elseif (request()->has('membershipId') && request()->has('membershipType') && request()->has('displayName')) {
            $aPlayers[request()->input('displayName')] = new DestinyPlayer([
    ...
    private function formatToHashtag($strGamertag)
    {
        ...
        if ($iPos !== false) {
            $strGamertag = substr_replace($strGamertag, '#', $iPos, 1);
        }
    ...
    private function formatNoBnet($strGamertag)
    {
        ...
        if (count($aGamertag) > 1) {
            $strGamertag = str_replace('#'.end($aGamertag), '', $strGamertag);
        }
