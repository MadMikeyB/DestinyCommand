...
/**
 * Executes Bungie-backed command actions.
 */
class BungieProvider
{
    private DestinyClient $destiny;

    /**
     * Create a new Bungie provider instance.
     */
    public function __construct(?DestinyClient $destiny = null)
    {
        $this->destiny = $destiny ?? new DestinyClient;
    }

    /**
     * Prepare or fetch provider data for a command action.
     */
        ...
        switch ($oAction->endpoint) {
            case 'vendor':
                ...
                if ($bPrepare === true) {
                    ...
                    if (in_array($oCurrDate->dayOfWeek, [6, 0, 1], true) || ($oCurrDate->dayOfWeek === 5 && $oCurrDate->hour >= 17) || ($oCurrDate->dayOfWeek === 2 && $oCurrDate->hour < 17)) {
                        $bXurHere = true;
                    }
            ...
            case 'profile':
                ...
                if ($bPrepare === true) {
                    $aComponents = $oAction->options->params['components'] ?? [];
                    $aComponents = array_merge($aComponents, [200]); // 200 is characters, we always need this.
            ...
            case 'stats':
                ...
                $aParams = [];
                if (isset($oAction->options->modes)) {
                    $aParams['modes'] = $oAction->options->modes;
                }
                if (isset($oAction->options->groups)) {
                    $aParams['groups'] = $oAction->options->groups;
                }
                ...
                if ($bPrepare === true) {
                    if (isset($oAction->options->seperate) && $oAction->options->seperate === true) {
                        $aCharacters = [];
                        foreach ($aParameters['players'] as $oPlayer) {
                            $this->destiny->getProfile($oPlayer->membershipType, $oPlayer->membershipId, [200]);
                        }
                        $aProfiles = $this->destiny->get('getProfile');
