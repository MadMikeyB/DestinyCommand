...
class Query
{
    ...
    {
        $aConsoles = ['xbox' => 1, 'ps' => 2, 'xb' => 1, 'xb1' => 1, 'psn' => 2, 'playstation' => 2, 'ps4' => 2, 'pc' => 3, 'bnet' => 3, 'steam' => 3, 'bungienet' => 254];

        // Seperate query by spaces
        $strQuery = urldecode($strQuery);
        $strQuery = str_replace([',', '%20'], [';', ' '], $strQuery); // Use 1 seperator
        $aQuery = explode(' ', $strQuery);
        $aQuery = array_diff($aQuery, ['']);
